...
describe Arara::Question do
  describe ".find_all_by_category_id" do
    ...
    before{ stub_request(:get, Arara::Question::URL).with(:query => {:token => ENV["VOC_TOKEN"], :category_id => 1}).to_return(:body => questions_page1) }
    before{ stub_request(:get, Arara::Question::URL).with(:query => {:token => ENV["VOC_TOKEN"], :category_id => 1, :page => 2}).to_return(:body => questions_page2) }
    before{ stub_request(:get, Arara::Question::URL).with(:query => {:token => ENV["VOC_TOKEN"], :category_id => 1, :by_updated_at => time}).to_return(:body => questions_page3) }
